...
# We set RmiPort=4447 (unique port, exposed in docker-compose as 4447:4447) so
# the JRMP transport port embedded in the NamingServer_Stub matches the host
# port mapping (127.0.0.1:4447 → container:4447).  This lets jnp-scan and
# jnp-exploit extract the port from the JNP bootstrap and connect directly.
# JNP bootstrap port stays at 1099 (mapped to host:4444).
echo "[CONFIGURE] Patching JNP bind address and RmiPort ..."
...
if [ -f "$JBOSS_SVC" ]; then
    ...
    # Use port 4447 for JRMP transport (avoids conflict with rmi-java8:1098 on host)
    sed -i 's|<attribute name="RmiPort">.*</attribute>|<attribute name="RmiPort">4447</attribute>|g' \
        "$JBOSS_SVC" 2>/dev/null || true
fi
