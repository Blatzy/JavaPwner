...
/**
 * Vulnerable Java RMI server — Java 11 variant.
 *
 * Exported objects:
 *   HelloService  → RMI_HELLO_PORT    (default 1198)
 *   DataService   → RMI_DATA_PORT     (default 1197)
 *   Registry      → RMI_REGISTRY_PORT (default 1199)
 *
 ...
 * dgcFilter field in DGCImpl — compatible with Java 11–21.
 * Requires --add-opens java.rmi/sun.rmi.transport=ALL-UNNAMED
 *           --add-opens java.base/sun.misc=ALL-UNNAMED
 *
 * Attack surface:
 *   javapwner rmi scan 127.0.0.1 -p 1199
 *   javapwner rmi exploit 127.0.0.1 -p 1199 --gadget CommonsCollections6 --cmd '...'
 */
public class RmiServer {

    public static void main(String[] args) throws Exception {
        int registryPort = getEnvInt("RMI_REGISTRY_PORT", 1199);
        int helloPort    = getEnvInt("RMI_HELLO_PORT",    1198);
        int dataPort     = getEnvInt("RMI_DATA_PORT",     1197);
        ...
        System.out.println("[RMI-JAVA11] ================================================");
        System.out.println("[RMI-JAVA11]  Vulnerable RMI Registry — Java 11 Lab Target");
        System.out.println("[RMI-JAVA11] ================================================");
        System.out.println("[RMI-JAVA11]  Java version : " + System.getProperty("java.version"));
        System.out.println("[RMI-JAVA11]  Hostname     : " + hostname);
        System.out.println("[RMI-JAVA11]  Registry port: " + registryPort);

        // Confirm CommonsCollections presence (gadgets need this)
        try {
            Class.forName("org.apache.commons.collections.Transformer");
            System.out.println("[RMI-JAVA11]  commons-collections 3.1 found (gadgets enabled)");
        } catch (ClassNotFoundException e) {
            System.out.println("[RMI-JAVA11]  WARNING: commons-collections not found in classpath!");
        }
        ...
        System.out.println("[RMI-JAVA11]  HelloService on port " + helloPort);
        System.out.println("[RMI-JAVA11]  DataService  on port " + dataPort);
        System.out.println("[RMI-JAVA11] ================================================");
        System.out.println("[RMI-JAVA11]  Ready — DGC channel open for exploitation");
        System.out.println("[RMI-JAVA11] ================================================");
    ...
    private static void disableDgcFilterUnsafe() {
        try {
            ...
            System.out.println("[RMI-JAVA11]  DGC filter : DISABLED (AllowAll via Unsafe)");
        } catch (NoSuchFieldException e) {
            // Java < 8u121: no JEP290 filter at all — already fully vulnerable
            System.out.println("[RMI-JAVA11]  DGC filter : absent (pre-JEP290 JVM)");
        } catch (Exception e) {
            System.out.println("[RMI-JAVA11]  DGC filter : bypass failed — " + e);
        }
    }
}
