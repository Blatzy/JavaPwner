...
/**
 * Vulnerable Java RMI server — Java 17 variant.
 *
 * Exported objects:
 *   HelloService  → RMI_HELLO_PORT    (default 1298)
 *   DataService   → RMI_DATA_PORT     (default 1297)
 *   Registry      → RMI_REGISTRY_PORT (default 1299)
 *
 ...
 * dgcFilter field in DGCImpl — compatible with Java 17–21.
 * Requires --add-opens java.rmi/sun.rmi.transport=ALL-UNNAMED
 *           --add-opens java.base/sun.misc=ALL-UNNAMED
 *
 * Attack surface:
 *   javapwner rmi scan 127.0.0.1 -p 1299
 *   javapwner rmi exploit 127.0.0.1 -p 1299 --gadget CommonsCollections6 --cmd '...'
 */
public class RmiServer {

    public static void main(String[] args) throws Exception {
        int registryPort = getEnvInt("RMI_REGISTRY_PORT", 1299);
        int helloPort    = getEnvInt("RMI_HELLO_PORT",    1298);
        int dataPort     = getEnvInt("RMI_DATA_PORT",     1297);
        ...
        System.out.println("[RMI-JAVA17] ================================================");
        System.out.println("[RMI-JAVA17]  Vulnerable RMI Registry — Java 17 Lab Target");
        System.out.println("[RMI-JAVA17] ================================================");
        System.out.println("[RMI-JAVA17]  Java version : " + System.getProperty("java.version"));
        System.out.println("[RMI-JAVA17]  Hostname     : " + hostname);
        System.out.println("[RMI-JAVA17]  Registry port: " + registryPort);

        // Confirm CommonsCollections presence (gadgets need this)
        try {
            Class.forName("org.apache.commons.collections.Transformer");
            System.out.println("[RMI-JAVA17]  commons-collections 3.1 found (gadgets enabled)");
        } catch (ClassNotFoundException e) {
            System.out.println("[RMI-JAVA17]  WARNING: commons-collections not found in classpath!");
        }
        ...
        System.out.println("[RMI-JAVA17]  HelloService on port " + helloPort);
        System.out.println("[RMI-JAVA17]  DataService  on port " + dataPort);
        System.out.println("[RMI-JAVA17] ================================================");
        System.out.println("[RMI-JAVA17]  Ready — DGC channel open for exploitation");
        System.out.println("[RMI-JAVA17] ================================================");
    ...
    private static void disableDgcFilterUnsafe() {
        try {
            ...
            System.out.println("[RMI-JAVA17]  DGC filter : DISABLED (AllowAll via Unsafe)");
        } catch (NoSuchFieldException e) {
            // Java < 8u121: no JEP290 filter at all — already fully vulnerable
            System.out.println("[RMI-JAVA17]  DGC filter : absent (pre-JEP290 JVM)");
        } catch (Exception e) {
            System.out.println("[RMI-JAVA17]  DGC filter : bypass failed — " + e);
        }
    }
}
